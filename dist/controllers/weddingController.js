...
exports.createWeddingUser = exports.createWedding = exports.getWeddings = void 0;
...
const getWeddings = async (req, res) => {
    try {
        const weddings = await Wedding_1.default.find().sort({ date: 1 });
        res.json(weddings);
    }
    catch (err) {
...
const createWedding = async (req, res) => {
    ...
    try {
        ...
        const existingWedding = await Wedding_1.default.findOne({ code });
        if (existingWedding) {
        ...
        if (Array.isArray(customEvents) && customEvents.length > 0) {
            defaultEvents = customEvents.map((evtName, index) => {
...
const createWeddingUser = async (req, res) => {
    ...
    try {
        ...
        const wedding = await Wedding_1.default.findById(id);
        if (!wedding) {
        ...
        await (0, mailer_1.sendWelcomeEmail)(email, name, role, wedding.name, password);
        res.status(201).json({
            id: user._id,
            name: user.name,
            email: user.email,
...
exports.createWeddingUser = createWeddingUser;
